...
const FINDER_FILE_DRAG_PREVIEW_ICON_SIZE: f64 = 32.0;
...
fn centered_image_rect(current_position: NSPoint, image_size: NSSize) -> NSRect {
    NSRect::new(
        NSPoint::new(
...
unsafe fn load_fallback_preview_image(image: Image) -> crate::Result<id> {
...
unsafe fn load_finder_file_icon(path: &std::path::Path) -> Option<id> {
    let workspace: id = msg_send![class!(NSWorkspace), sharedWorkspace];
    if workspace == nil {
        return None;
    }

    let icon: id = msg_send![workspace, iconForFile: NSString::new(&path.display().to_string()).0];
    if icon == nil {
        return None;
    }

    let preview_size = NSSize::new(
        FINDER_FILE_DRAG_PREVIEW_ICON_SIZE,
        FINDER_FILE_DRAG_PREVIEW_ICON_SIZE,
    );
    let _: () = msg_send![icon, setSize: preview_size];
    Some(icon)
}
...
pub fn start_drag<W: HasWindowHandle, F: Fn(DragResult, CursorPosition) + Send + 'static>(
    ...
    if let Ok(RawWindowHandle::AppKit(w)) = handle.window_handle().map(|h| h.as_raw()) {
        unsafe {
            ...
            let fallback_img = load_fallback_preview_image(image)?;
            let fallback_image_size: NSSize = fallback_img.size();
            let fallback_image_rect = centered_image_rect(current_position, fallback_image_size);
            ...
            match item {
                DragItem::Files(files) => {
                    for path in files {
                        ...
                        let drag_preview_img =
                            load_finder_file_icon(&path).unwrap_or(fallback_img);
                        let drag_preview_size: NSSize = drag_preview_img.size();
                        let drag_preview_rect =
                            centered_image_rect(current_position, drag_preview_size);

                        let _: () = msg_send![item, setDraggingFrame: drag_preview_rect contents: drag_preview_img];
                ...
                DragItem::Data { provider, types } => {
                    ...
                    let _: () = msg_send![item, setDraggingFrame: fallback_image_rect contents: fallback_img];
