...
fn find_existing_item_by_titles(menu: &NSMenu, titles: &[&str]) -> Option<Retained<NSMenuItem>> {
    for title in titles {
...
fn menu_target_class() -> &'static AnyClass {
    ...
    CLASS.get_or_init(|| {
        ...
        let mut builder = ClassBuilder::new(c"VJDownloaderMenuTarget", superclass).expect("class");
        unsafe {
