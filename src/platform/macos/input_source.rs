...
pub fn current_mode() -> Option<InputMode> {
    ...
    let input_source_id =
        unsafe { cf_string_to_rust(TISGetInputSourceProperty(source, kTISPropertyInputSourceID)) }
            .unwrap_or_default();

    let localized_name =
        unsafe { cf_string_to_rust(TISGetInputSourceProperty(source, kTISPropertyLocalizedName)) }
            .unwrap_or_default();
