...
- 起動時のフルスキャンはメイン画面右上、設定画面から開始したフルスキャンは設定画面の検索対象フォルダ欄に進行中表示を出す。DBへの反映完了後は同じ場所にチェックマークのアニメーションと
  `インデックスを更新しました`という完了表示を一定時間表示し、失敗時はエラー表示へ切り替える。
...
- インデックス作成時に同梱`ffprobe`で動画の`comment`および`description`タグを取得し、検索用コメントとして結合する。
- ファイルサイズと更新日時が前回インデックス時から変わっていない場合、保存済みコメントを再利用して`ffprobe`の再実行を省略する。
- `ffprobe`が30秒以内に応答しない場合はそのファイルのコメント取得を打ち切り、警告ログを残して次のファイルへ進む。
