...
- 非空クエリでは2段階検索を行う。
- キーワードが1つの場合、第1段階は前方一致（`query%`）で検索し、足りない場合に第2段階の部分一致（`%query%`）で補完する。
...
- Windows実装は現時点では公開APIの形のみを揃えた未実装スタブであり、フォルダ選択・IME判定・アプリメニューは機能しない。実装すべき内容は各ファイルの`TODO(windows)`に記載する。
- `cargo check`は実行中のOS側のみを検証する。もう一方のターゲットの検証はCIで行う。

## 実装デフォルト値と変更方法

- 検索正規化方式: `normalize_for_search`（NFKC + lower + カタカナのひらがな化）
