...
- `files`には`path`（PK）、`root_id`、`file_name`、`file_name_norm`、`comment`、`comment_norm`、`parent_dir`、`size_bytes`、`modified_time`、`created_time`、`last_indexed_time`を保持する。
...
- 起動時のフルスキャンはメイン画面右上、設定画面から開始したフルスキャンは設定画面の検索対象フォルダ欄に進行中表示を出す。DBへの反映完了後は同じ場所にチェックマークのアニメーションと`インデックスを更新しました`という完了表示を一定時間表示し、失敗時はエラー表示へ切り替える。
