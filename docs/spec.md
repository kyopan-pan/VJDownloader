# VJDownloader 仕様

## アプリ全体

### 概要

- クリップボードのURLから動画をダウンロードし、ローカルのMP4を管理・送出するmacOS/Windows向けツール。
- UIはダーク基調で、ダウンロード進捗と保存済みリストを表示する。
- アプリ名は「VJDownloader」で表示される。
- macOSアプリバンドルのアイコンは`assets/icon/App.icns`をそのまま同梱して使用する（ビルド時のアイコン生成は行わない）。
- Windowsの実行ファイルのアイコンは`assets/icon/App.ico`を`build.rs`でexeへリソースとして埋め込む。
  Windowsにはアプリバンドルに相当する外付けの指定手段が無く、エクスプローラやタスクバーはexe内のリソースを参照する。
  埋め込みはWindowsホストでビルドしたときのみ行い、失敗した場合はビルドエラーとする。
- `assets/icon/App.ico`は16/24/32/48/64/128/256pxの7解像度を持つ。
  `assets/icon/App.iconset`から`scripts/build-windows-icon.sh`で生成する。
- 実行中のウィンドウおよびタスクバーのアイコンは、Windowsでは`assets/icon/App.iconset/icon_256x256.png`を
  実行ファイルへ埋め込んで実行時に設定する。設定しない場合はeframe既定のeguiロゴが表示される。
  macOSはアプリバンドルのアイコンを使うため、実行時のアイコン設定は行わない。
- アプリアイコンはmacOSのアイコングリッドに従い、1024pxのキャンバスに対して本体を824px（80.5%）とし、
  周囲に各辺100pxの透明な余白を持たせる。余白が不足するとDockやFinderで他アプリより大きく表示される。

### ウィンドウ

- 初期サイズは幅420px・高さ720px。
- 最小サイズは幅360px・高さ640px。

### テーマとフォント

- ダークテーマを適用する。
- ベース背景色は`rgb(12, 18, 32)`を使用する。
- 主要なアクセントカラーは`rgb(16, 190, 255)`を使用する。
- ボタンやパネルは角丸を使用する。
- フォントはSF系フォントを優先し、無い場合はAvenir系を使用する。
- 日本語フォントはHiragino Sans等のシステムフォントから順に使用する。
- Windowsでは`SystemRoot`（未設定時は`WINDIR`、さらに未設定なら`C:\Windows`）配下のFontsから游ゴシック、メイリオ、MS ゴシックの順に読み込み、比例・等幅フォント双方の先頭に登録し、英字と日本語を同じフォントで描画してベースラインを揃える。macOSのフォント選択は変更しない。

### エラーハンドリング

- 保存先作成失敗時はダウンロードを中断し、エラーを表示する。
- yt-dlp起動失敗時はダウンロード失敗として扱う。
- ファイル削除失敗時はステータスに`削除に失敗しました: <error>`を表示する。
- ドラッグ開始失敗時はステータスにエラーを表示する。
- Bot対策の`拒否`を検出したまま失敗した場合は、通常のエラーログに加えて制限状態をログとUIに表示する。

## 設定とデータ

### 保存先と設定

- 既定の保存先は`~/Movies/VJDL`。
- 設定ファイルは`~/.vjdownloader/settings.properties`。
- 設定キー`download.dir`が存在し空でない場合、その値を保存先として使用する。
- 設定キー`download.mode`にダウンロード仕様を保存する。値は`standard`/`up_to_1080p`/`best_then_convert`のいずれかで、
  未設定または未知の値の場合は`standard`として扱う。
- 設定キー`layout.download.width`/`layout.search.width`にメイン画面の左右ペイン幅を保存する。
- 設定ファイルは`#`または`!`で始まる行をコメントとして無視する。
- 設定ファイルは`key=value`または`key:value`形式の行のみを読む。

### 設定画面

- `Cmd+,`でも設定画面を開ける。
- macOSのメニューバー（Appメニュー）から`設定...`を開ける。
- `Cmd+L`またはmacOSのメニューバー（Appメニュー）`ログ...`でログ画面を開ける。
- 設定画面は独立したウィンドウとして表示する。
- 出力先フォルダ、ダウンロード仕様、YouTube認証（ブラウザクッキー）の設定を編集できる。
- ダウンロード仕様は3つの選択肢をラジオボタンで切り替える。
- 各選択肢の説明と注意書きは画面に常時表示せず、選択肢へカーソルを合わせたときの吹き出しで表示する。
- 各セクションのパネルは設定画面の横幅いっぱいに揃える。
- 出力先フォルダはボタンからmacOSのフォルダ選択UIで指定できる。
- クッキー利用時はブラウザ名が必須で、未入力の場合は保存できない。
- 保存時に出力先フォルダが存在しない場合は作成を試みる。
- yt-dlp/Denoのバージョンとステータスを表示し、`最新を取得`で再取得できる。

### クッキー設定

- 設定キー`cookies.from_browser.enabled`が`true`のときのみクッキー取得を有効化する。
- 設定キー`cookies.from_browser.browser`が空の場合はクッキー取得を無効扱いとする。
- 設定キー`cookies.from_browser.profile`が空でない場合は`browser:profile`形式を使用する。
- クッキー取得はyt-dlpの`--cookies-from-browser`オプションとして渡す。

### 内部パス

- アプリ用データは`~/.vjdownloader`配下を使用する。
- `~/.vjdownloader/bin`にツール用のバイナリを配置する。
- yt-dlpは`~/.vjdownloader/bin/yt-dlp`に保存する。
- ffmpegは`~/.vjdownloader/bin/ffmpeg`を参照する。
- ffprobeは`~/.vjdownloader/bin/ffprobe`を参照する。
- denoは`~/.vjdownloader/bin/deno`を参照する。

## 外部ツールのセットアップ

### 初回セットアップ画面

- yt-dlpまたはDenoが未導入・実行不可の場合に初回セットアップ画面を表示する。
- 初回セットアップ画面は独立したウィンドウとして表示する。
- 初回セットアップ画面でyt-dlp/Denoの状態とバージョンを表示する。
- `自動セットアップ`でyt-dlp/Denoを取得し、完了後に状態を更新する。

### Windowsの初回セットアップ

- yt-dlpとDenoはWindowsの対象CPU（x64/ARM64）向け公式バイナリを取得し、`.exe`名で検証・保存・検出する。
- DenoのZIPはWindows PowerShellから呼び出す.NETの`ZipFile::ExtractToDirectory`で展開する。パスは環境変数で渡し、空白・日本語・記号を含むパスを保護する。
- macOSの取得先と展開方式は維持する。
- ffmpeg/ffprobeはBtbN/FFmpeg-Buildsの静的ビルドZIP（`latest`タグ）を対象CPU別に取得する。
  x64は`ffmpeg-master-latest-win64-gpl.zip`、ARM64は`ffmpeg-master-latest-winarm64-gpl.zip`を使用する。
- ZIPには`ffmpeg.exe`と`ffprobe.exe`の両方が含まれるため、ダウンロードは1回にまとめる。
  実行ファイルは`<展開名>/bin/`配下に置かれるため、展開先を再帰的に探索して取り出す。
- 配置先はアプリ用binフォルダに固定し、PATH上のffmpegは書き換えない。
- 取得手順はyt-dlp/denoと同じく「一時フォルダへダウンロード → 内容を検証 → 本体を置き換え → 前バージョンを削除」とする。
  片方だけが新しい状態にならないよう、ffmpegとffprobeの両方の検証が終わってから置き換える。
- 取得はUIを止めないよう、起動時のバックグラウンド処理で行う。ダウンロード開始時に未導入だった場合はその時点でも取得を試み、
  進捗をログへ出力する。起動時取得とダウンロード開始時取得は排他制御し、二重取得を防ぐ。
- 起動時の取得に失敗した場合はログへ理由を`ERROR`で出力する。対象はyt-dlp・Deno、Windowsではffmpeg/ffprobeも含む。

## ダウンロード

### ダウンロード開始

- ダウンロード開始はクリップボードの文字列をそのままURLとして利用する。
- クリップボードに文字列がない、または空の場合は何もしない。

### ダウンロード処理

- ダウンロードは別スレッドで実行する。
- 起動時にバックグラウンドでyt-dlp/denoの有無を確認し、未導入ならGitHubの最新リリースから取得する。
- yt-dlpをダウンロードした後、実行権限を付与する。
- macOSではffmpeg/ffprobeを同梱バイナリから`~/.vjdownloader/bin`へコピーし、実行権限を付与する。
- macOSの同梱ffmpeg/ffprobeは外部ライブラリを含まない自前ビルド（LGPL-2.1-or-later、最小macOS 13.0、arm64）で、
  ビルド手順は`scripts/build-ffmpeg-macos.sh`にある。
- WindowsではmacOS用の同梱ffmpeg/ffprobeを配置しない。`ffmpeg.exe`/`ffprobe.exe`はアプリ用binフォルダ、PATHの順に探索し、Windows PE形式かつ`-version`を正常実行できるものだけを使用する。
- Windowsで使用可能な`ffmpeg.exe`/`ffprobe.exe`が見つからない場合は、初回セットアップで公式ビルドを取得する。
- ffmpeg/ffprobeの探索結果は、使用可能と確定した場合のみ記憶する。見つからなかった場合は記憶せず次回も探索し直し、
  アプリ起動後にffmpegを導入した場合も再起動せずに復旧できるようにする。
- denoが存在しない場合はGitHubの最新リリースから`deno-aarch64-apple-darwin.zip`をダウンロードし展開する。
- yt-dlp/denoの取得と更新は「一時フォルダへダウンロード → 内容を検証 → 本体を置き換え → 前バージョンを削除」の順で行い、
  ダウンロード中も既存バイナリを残す。
- 検証はファイルサイズ（1MiB以上）と`--version`の実行成功で判定し、失敗した場合は前バージョンを維持する。
- 既存バイナリが不完全な場合や、旧仕様で退避されたままのバイナリがある場合は、本体として復元してから再取得を判断する。
- yt-dlpが実行可能でない場合はダウンロードを開始しない。
- 保存先フォルダが存在しない場合は作成する。
- 出力テンプレートは`%(title)s.%(ext)s`を使用する。
- yt-dlp実行時に`~/.vjdownloader/bin`をPATH先頭に追加する。
- yt-dlpのstdout/stderrはUTF-8で行単位に読み取り、ログと進捗に反映する。
- yt-dlpの生の進捗行はそのまま大量に表示せず、5%刻みの`ダウンロード進捗: n%`へ集約する。
  集約対象は`[download]`タグの直後が数値と`%`である行に限り、ファイル名に`%`を含む通知行はそのままログへ出力する。
- ダウンロード中にStopを押した場合は実行中のプロセスを終了してキャンセルする。

### ダウンロード仕様（3つのオプション）

- 設定画面で選んだダウンロード仕様に従ってyt-dlpの引数と後処理を切り替える。
- ダウンロード開始時に、選択中のダウンロード仕様をログへ出力する。
- ダウンロード仕様の変更は次回のダウンロードから適用する。
- AnimeThemes専用パイプラインはダウンロード仕様の選択に影響されない。
- `標準（H.264優先）`（`standard`）: 従来どおりH.264優先モードで取得し、失敗した場合のみ互換モード（720p上限）で再試行する。
- `1080p上限`（`up_to_1080p`）: 1080pを上限にH.264優先モードで取得する。1080p未満が最高画質の場合はその画質を使う。
  失敗した場合は1080p上限の互換モードで再試行する。
- `最高画質 + 変換`（`best_then_convert`）: コーデックを問わず最高画質で取得し、その後に既定フォーマット（H.264 MP4）へ変換する。
  互換モードでの再試行は行わない。

### ダウンロードオプション（共通）

- `--no-playlist`を指定する。
- Windowsを含めて日本語の動画名・パスを正しくログ表示するため、`--encoding utf-8`を指定する。
- 進捗出力を安定して行単位で取得し、更新頻度を抑えるため、`--newline`と`--progress-delta 1`を指定する。
- `--extractor-args youtube:player_client=web`を指定する。
- `--extractor-args youtube:skip=translated_subs`を指定する。
- `--concurrent-fragments 4`を指定する。
- `--ffmpeg-location`を指定する。
- `--js-runtimes deno`を指定する。

### ダウンロードオプション（優先モード）

- `標準`と`1080p上限`で使用する。
- `-S vcodec:h264,res,acodec:m4a`を指定する。
- `--match-filter vcodec~='(?i)^(avc|h264)'`を指定する。
- `--merge-output-format mp4`を指定する。
- `1080p上限`の場合は`-f bv*[height<=1080]+ba/b[height<=1080]`を追加で指定する。
- 優先モードが失敗した場合は互換モードで再試行する。

### ダウンロードオプション（互換モード）

- `-f bv*[height<=N]+ba/b[height<=N]`を指定する。Nは`標準`で720、`1080p上限`で1080。
- `--recode-video mp4`を指定する。
- `--postprocessor-args VideoConvertor:-c:v <H.264エンコーダ> -b:v 5M -pix_fmt yuv420p`を指定する。

### ダウンロードオプション（最高画質 + 変換）

- `-f bv*+ba/b`を指定し、コーデックの絞り込み（`-S`/`--match-filter`）は行わない。
- 取得したコーデックでも結合できるよう`--merge-output-format mkv`を指定する。
- ダウンロード完了後、一時フォルダ内の動画ファイルを同梱ffmpegで既定フォーマット（H.264 MP4）へ変換する。
  変換設定は変換ウィンドウと共通で、`h264_videotoolbox`（Windowsは失敗時に`libx264`へフォールバック）・`-b:v 5M`・
  `yuv420p`・AAC 192k・`faststart`を使用する。
- 変換中の出力は一時フォルダ内の隠しファイル（`.<元のファイル名>.converting`）とし、成功後に`<元のファイル名の拡張子なし>.mp4`へ確定して変換元を削除する。
- 変換フェーズでは進捗表示を`変換中...`に切り替え、ffmpegのログをログへ出力する。
- 変換に失敗した場合はダウンロード失敗として扱い、一時ファイルを削除する。

### AnimeThemes専用パイプライン

- URLに`animethemes.moe`を含む場合に専用パイプラインへ分岐する。
- ファイル名はURLパスを基にした`.mp4`（タイムスタンプ付き）を使用する。
- URL解析・API/HTML確認中は進捗メッセージに`動画情報確認中・・・`を表示する。
- パイプラインは`リンク取得 -> 直リンク(webm)受信とffmpeg変換(mp4)の並列実行`を基本とする。
- 直リンク取得（優先）: `AnimeThemes API`（`/anime/<slug>?include=animethemes.animethemeentries.videos`）を優先し、必要に応じて
  `/anime?filter[slug]=<slug>&include=...`も試行する。
- APIレスポンスはJSON:API形式（`included` + `relationships`）と従来のネスト形式の両方に対応し、
  `theme.slug/type+sequence -> animethemeentries -> videos -> link`を辿って`.webm`を抽出する。
- APIで取得できない場合はHTML解析へフォールバックし、`curl -sL -m 8 -A <UA> --range 0-262143`で先頭を取得して`og:video`
  または`video src`から`https://.../*.webm`を抽出する。見つからない場合は全文取得で再試行する。
- 直リンクを取得できた場合はRust HTTPクライアントでGETを直ちに開始し、レスポンス本文を`ffmpeg`の`stdin`
  へ逐次転送してダウンロードと変換を同時進行させる。事前のHEADリクエストは行わない。
- 直リンク経路のダウンロード進捗は、同じGETレスポンスの`Content-Length`と転送量から算出し、受信中に`n%`を表示する。
  `Content-Length`がない場合は受信済みMBを表示する。
- ダウンロード進捗は進捗バーだけでなくログにも`ダウンロード進捗: n%`として出力する。
- ffmpeg変換はH.264エンコーダを必須とし、ffmpegの`-encoders`に存在しない場合は処理を中断する。
  macOSではさらにApple Silicon（aarch64）であることを必須とする。
- ffmpeg変換ログは整形せずデフォルト出力をそのままログへ出力する。
- 直リンク取得に失敗した場合、または直リンク経路の`curl`/`ffmpeg`処理が失敗した場合は
  `yt-dlp --no-playlist --encoding utf-8 --newline --progress-delta 1 --concurrent-fragments 4 -f "bv+ba/b" --ffmpeg-location <ffmpeg> -o - <ページURL>`
  の出力をffmpegへパイプする。
- ffmpegは
  `-stats -analyzeduration 100M -probesize 100M -c:v <H.264エンコーダ> -b:v 5M -pix_fmt yuv420p -c:a aac -b:a 192k -ignore_unknown -movflags +faststart -f mp4 -y <出力パス>`
  を基本とし、直リンク経路・yt-dlpフォールバック経路ともに`-f webm -i pipe:0`を使用する。

### 進捗表示

- 進捗パネルは常に表示され、待機中は半透明表示となる。
- 進捗メッセージの初期値は`待機中...`。
- ダウンロード開始直後は`動画読み込み中...`を表示する。
- 進捗率が取得できる場合は`ダウンロード中... xx.x%`を表示する。
- 変換や結合が始まった場合は`変換中...`を表示する。
- 完了時は`ダウンロード完了!`を表示する。
- 完了後1.2秒で進捗表示を非表示 (待機状態)に戻す。
- 進捗率が不明な場合はインジケータをアニメーション表示する。

### 進捗の判定

- yt-dlp出力に`[merger]`や`[ffmpeg]`などの語が出現した場合は変換フェーズと判定する。
- yt-dlp出力中の`%`表記から進捗率を抽出する。進捗率が100%でも変換中には切り替えない。

## Bot対策

### Bot対策の検出

- yt-dlp/ffmpegの出力を1行ずつ判定し、Bot対策の兆候を`拒否`と`遅延`の2種類に分類する。
- `拒否`の判定は、`error`／`warning`／`unable`／`failed`／`giving up`のいずれかを含むエラー文脈の行に限定する。
  これにより、動画タイトルやファイル名に同じ語が含まれても誤検出しない。
- `拒否`と判定するのは以下の行。
  - `Sign in to confirm`または`not a bot`を含む行。
  - `captcha`を含む行。
  - HTTPステータスとして`403`または`429`が現れる行（`http error 403`、`returned error: 403`、`403 forbidden`、`status code: 429`など）。
  - `too many requests`を含む行。
  - `This content isn't available`を含む行。
- `遅延`と判定するのは以下の行。判定は`拒否`を優先する。
  - `sleeping`を含む行。`xx seconds`表記があれば待機秒数を読み取る。
  - `retrying`または`retry `を含む行。
  - `throttl`または`rate limit`を含む行。
- 数字だけの`403`／`429`（ファイル名など）は誤検出しないよう、HTTPステータスとして現れる形のみを判定対象とする。
- キャンセル中に検出した行は判定に使用しない。

### Bot対策のUI表示

- Bot対策の`遅延`または`拒否`を検出している間、進捗パネルを黄色の警告表示に切り替える。
  - 進捗バーと枠線を`rgb(250, 204, 21)`にする。
  - 進捗メッセージの下に`⚠ <検出内容>`を表示する。
  - 待機秒数を読み取れた場合は`Bot制限のためxx秒待機中・・・`を表示する。
  - 警告文はYouTubeのURLの場合のみ`YTの`を前置する（例:`⚠ YTのBot制限のため6秒待機中・・・`）。
- 警告表示は最後の検出から12秒間、待機秒数が分かる場合は`待機秒数+3秒`と12秒の長い方だけ維持する。
- Bot対策の`拒否`を検出したままダウンロードが失敗した場合、メインボタンを`rgb(220, 38, 38)`にして
  YouTubeのURLなら`YouTube制限中`、それ以外なら`サイト制限中`と表示する。
- 赤ボタン表示中はボタン下に`<検出内容>。数分待ってから再試行してください。`を表示し、同じ内容をログにも出力する。
- 赤ボタン表示は10分経過するか、次のダウンロードを開始すると解除する。
- 赤ボタン表示中もボタンは有効で、クリックすると通常どおり再ダウンロードを開始する。
- キャンセルによる失敗はBot対策由来の失敗として扱わない。

## MP4変換

- macOSのメニューバー（Appメニュー）`動画をMP4に変換...`から、独立した変換ウィンドウを開ける。
- 変換ウィンドウへWebM、MOV、MKVなどの動画ファイルをドラッグ＆ドロップすると、同梱ffmpegによるMP4変換を自動的に開始する。
- 複数ファイルを同時にドロップした場合は、ドロップされたファイルを順番に変換する。
- 変換中に追加のファイルがドロップされた場合は受け付けず、現在の変換が終わってから再度ドロップするよう案内する。
- 映像は`h264_videotoolbox`（macOS）または`libx264`（Windows）、音声はAACへ変換し、`yuv420p`および`faststart`を指定する。
- Windowsでは`h264_videotoolbox`での変換が失敗した場合に`libx264`へ自動的にフォールバックする。
  macOSの同梱ffmpegは`libx264`を含まないLGPLビルドのためフォールバックせず、失敗をそのまま報告する。
- 変換中の一時ファイルはダウンロード一覧に現れない隠しファイル名とし、成功後に最終的な`.mp4`名へ変更する。
- 変換済みファイルは設定済みの出力先フォルダへ保存し、同名ファイルがある場合は`-converted`と連番を付けて上書きを避ける。
- 変換完了後はメイン画面のダウンロードリストを即時更新し、変換したMP4を表示する。
- 変換開始、エンコーダ切り替え、ffmpegの標準エラー出力、変換の成功・失敗をログへ逐次記録する。
- ffmpegの進捗行（`frame=`/`size=`で始まるstats行）は5秒に1件までに間引く。間引きはログ出力のみが対象で、
  失敗時のエラー本文には全行を残す。

## ストリーム再生

### 画面と再生方式

- macOSのメニューバー（Appメニュー）`ストリーム再生...`でストリーム再生画面を開ける。
- Syphon出力はストリーム再生画面の表示開始時から常時有効とし、有効・無効を切り替えるチェックボックスは設けない。
- ストリーム再生画面を閉じるとSyphonサーバーを破棄して送信を停止し、画面を再度開いたときに再初期化して送信を再開する。
- Syphon出力の配信状態、受信側の接続有無、送信フレーム数、初期化エラーは画面内に表示する。
- ストリーム再生画面は独立したウィンドウ（eguiビューポート、初期サイズ560x560、最小480x480）として表示する。
- ストリーム再生画面はウィンドウ内に16:9のミニプレビューを配置し、そこで映像を再生する。
- `ストリーム開始`ボタンはダウンロードと同様にクリップボードの文字列をそのままURLとして利用する。
- クリップボードに文字列がない場合はエラーを表示し、再生は開始しない。
- 再生は外部プレイヤーを使わず、同梱ffmpegでデコードした映像をウィンドウ内テクスチャとして描画する。
- 再生画面に常設の状態表示ボックスは設けず、失敗時のみエラーメッセージを表示する。
- yt-dlp/ffmpegが見つからない場合はストリーム画面にエラーを表示する。

### URL解決と再生パイプライン

- AnimeThemes URLはyt-dlpを使用せず、AnimeThemes
  API（失敗時はHTML解析）からWebM直リンクを取得してffmpegへ直接渡す。並行キャッシュは作成せず、シーク・ループ時も同じ直リンクを利用する。
- 再生開始時にyt-dlpで総再生時間と直リンク（映像/音声URL）を取得する。
- yt-dlpは`--no-playlist`・`--encoding utf-8`・`youtube:player_client=web`・`youtube:skip=translated_subs`・`--js-runtimes <deno>`・
  `-f bv*[height<=480]+ba/b[height<=480]/b`・`--print "DURATION:%(duration)s"`・`-g`で実行し、標準出力から`DURATION:`行と
  `http(s)`のURL行を解析する。
- yt-dlp実行時は`~/.vjdownloader/bin`をPATH先頭に追加する。
- 初回は取得した直リンクを同梱ffmpegへ渡して直ちに再生し、それと並行してyt-dlpでデッキごとの一時ディレクトリへ動画全体を1回だけキャッシュする。
- キャッシュ完成前のループ・シークは取得済み直リンクを使用し、キャッシュ完成後のループ・シークはローカルファイルへ切り替える。キャッシュはループごとに書き直さない。
- ffmpegは映像を`fps=30`・`480x270`へ整形した生RGBA（`-f rawvideo -pix_fmt rgba pipe:1`）として標準出力へ送り、音声は
  `-f audiotoolbox -audio_device_index -1`でmacOSの既定音声デバイスへ実時間再生する。映像が2入力構成なら音声は2番目の入力、単一入力なら同一入力から取る。
- 音声側がAudioToolboxで実時間再生されるため、映像フレームの供給ペースも再生速度に揃う。
- 映像は固定フレームレート (30fps)でデコードし、各フレームにフレーム番号から算出した提示時刻 (PTS)を付与する。
- カクつき防止のため、受信フレームをジッタバッファに蓄積し、再生クロックに到達したフレームの最新1枚を提示する（クロック未確立のバッファリング中は最新フレームを即時提示）。
- 再生クロックはffmpegのstats (`time=`)で更新し、`-stats_period 0.1`で細かく取得したうえでstats間はウォールクロックで補間する。

### 再生コントロール

- プレビュー直下に`巻き戻し`・`再生/一時停止`・`早送り`のアイコンボタン行を常時表示する（文字ではなくベクター描画のアイコン）。
- `再生/一時停止`は、一時停止時に現在位置を確定してffmpegを終了し、再開時にその位置からffmpegを再起動する。再生中のみ一時停止アイコン、それ以外（停止中・一時停止中）は再生アイコンを表示する。
- 停止中は巻き戻し/早送り/一時停止の操作は無効（何もしない）。
- `巻き戻し`は現在位置から-0.5秒、`早送り`は+0.5秒の相対シークを行う（シークと同じくffmpegを再起動する）。
- 画面下部のメインボタンは、停止中は`ストリーム開始`、再生中は`ストリーム`を表示する。
- `ストリーム`/`ストリーム開始`ボタンはクリップボードのURLを読み、停止中は再生開始、再生中は現在の再生を新しい動画へ置き換える。クリップボードにURLが無い場合は現在の再生を継続する。
- 動画が正常に最後まで再生された場合は、完成済みのローカルキャッシュを再利用して冒頭から再生を再開し、動画が差し替えられるか再生画面が閉じられるまでループ再生を継続する。
- 一時キャッシュは各デッキにつき1動画・1ファイルだけ保持し、動画の差し替え、再生停止、またはストリーム再生画面の終了時に削除する。
- ストリーム再生画面を閉じると再生を終了し、実行中のプロセス・プレビュー・再生位置・ローカルキャッシュを初期化する。
- 再生終了時はプレビューを消去する。

### シークバー

- シークバーはウィンドウ内に現在の再生位置と総再生時間を`現在 / 総時間`形式で表示する。
- 総再生時間が取得できている再生中はシークバーをドラッグまたはクリックして任意位置へシークできる。
- シーク確定時は再生世代を更新して現行ffmpegを終了し、キャッシュ完成前は取得済み直リンク、完成後はローカルキャッシュから指定位置で再起動する。
- 再生位置はffmpegのstats出力（`time=`）を解析して更新し、stats間はウォールクロックで補間する。
- 総再生時間が不明（ライブ等）の場合はシーク不可とし、経過時間のみ表示する。

### 再生世代の管理

- 各再生（初回・シーク）に通し番号 (run_id)を割り当て、停止・シーク時は番号を更新する。
- フレーム/再生位置/終了通知は現行run_idのもののみ反映し、終了した世代の通知は破棄する。

## ファイル一覧とUI

### ファイル一覧

- 保存先フォルダ内の`.mp4`のみを表示する。
- 一覧は最終更新日時の降順で並べる。
- 一覧の表示高は360pxで固定する。
- リストが空の場合は`まだダウンロードがありません。`を表示する。
- 行右端の×印ボタンで削除できる。×印はフォントに依存しない線画として表示する。
- ファイル名は左寄せで表示する。
- ファイル名は12pt、字間-1ptで表示し、狭いウィンドウでも表示文字数を確保する。
- ただし空白（半角・全角・連続を含む）の字間は+1ptとし、字間を詰めても単語の区切りが読み取れるようにする。
- ファイル名の上下パディングは等間隔に揃える。
- ファイル名が長い場合は末尾を`...`で省略する。

### Drag & Drop

- リスト項目のドラッグでOSネイティブのファイルドラッグを開始する。
- ドラッグ中はファイルアイコンが表示される。
- ドロップ先へはファイル参照が渡る。
- ファイル名を含むホバーでハイライトされる範囲全体がドラッグ対象。
- ホバー時はマウスカーソルをポインタ表示に変更する。
- 単クリックではドラッグを開始しない。
- ドラッグは押下したまま移動したときに開始する。
- ドラッグ開始時はファイルパスを正規化し、失敗時はステータスにエラーを表示する。
- ドラッグ用アイコンは、macOSではシステムの書類アイコン、Windowsでは実行ファイルへ埋め込んだ32px PNG（`assets/icon/drag_32x32.png`）を使用し、実行時の作業ディレクトリに依存しない。macOSでシステムアイコンが見つからない場合も埋め込みPNGへフォールバックする。
- ドラッグ用アイコンはアプリアイコンとは別画像とする。Dock向けの余白はドラッグ画像には不要なため、余白を持たない画像を使用する。

### カーソル挙動

- メイン/設定/ログの全ウィンドウで、クリックまたはドラッグ可能な要素はホバー時にポインタ表示とする。
- 検索欄や設定のテキスト入力欄はI-beamカーソルを維持する。
- フォーカス喪失中およびフォーカス復帰直後もカーソル追従を維持する（OS制約内のベストエフォート）。

### UIテキスト

- メインボタンの表示は待機時`Download`、ダウンロード中は`Stop`、Bot対策で失敗した直後は`YouTube制限中`（YouTube以外は`サイト制限中`）。
- サブタイトルに`リストをドラッグしてVDMXへドロップ`を表示する。
- ダウンロード中もメインボタンは有効で、クリックするとキャンセルする。
- メイン画面の左右ペイン幅はアプリ終了時に保存し、次回起動時に復元する。
- 左右ペインの幅比は`1:1`を下限とし、ダウンロード側が検索側を上回らないようにする。

### ログの記録内容

- ログ1件は「発生時刻・重大度・発生源・本文」で構成する。項目名と値はOpenTelemetry Logs Data Modelに合わせる。
  分散トレースを行わないため`TraceId`/`SpanId`は持たない。
- 重大度は`DEBUG`(SeverityNumber 5)・`INFO`(9)・`WARN`(13)・`ERROR`(17)の4段とする。
- 発生源は`app`・`setup`・`download`・`convert`・`search`・`stream`・`speedtest`のいずれかとする。
- 本文が複数行になる場合（ffmpegの標準エラー出力、パニックの内容など）は改行を`\n`という文字列へ畳んで
  1件を必ず1行に収める。空行と行末の空白は落とす。
- 発生時刻はRFC 3339形式（ミリ秒・ローカルオフセット付き）で記録する。タイムゾーンを取得できない環境ではUTCで記録する。
- yt-dlpとffmpegの出力行は、行頭が`ERROR`なら`ERROR`、`WARNING`または`WARN`なら`WARN`、それ以外は`INFO`として扱う。
  変換ウィンドウのffmpeg出力（`[ffmpeg]`で始まる行）は件数が多いため`DEBUG`とする。
- 起動時に`VJDownloader <バージョン> を起動しました`を`INFO`で記録する。
- ワーカースレッドを含むパニックは`panicが発生しました [thread: <スレッド名>] <内容>`としてログファイルへ記録する。
  ログ画面のリングバッファは経由しない。パニック時はファイルへの書き込み完了を待ち、非同期writerが応答しない場合は同期追記へ退避する。
- 次の事象をログへ記録する。
  - `setup`: yt-dlp / Deno / ffmpeg（Windows）の取得開始・完了・失敗。設定画面から実行した場合も記録する。
  - `download`: ダウンロード仕様、進捗、外部ツールの出力、AnimeThemesの解決経路とフォールバック、成功・失敗。
  - `convert`: 変換開始、エンコーダ切り替え、ffmpegの出力、成功・失敗。
  - `search`: 検索インデックスのDB初期化・更新・全走査・フォルダ監視の失敗、動画コメント読み取りの失敗。
  - `stream`: URL解決の開始と結果、ffmpegの開始・終了・標準エラー出力、音声なしでの再試行、
    Syphon出力の初期化失敗、一時キャッシュ作成の失敗。
  - `speedtest`: 測定結果と失敗。
  - `app`: ファイル削除・ドラッグの失敗、入力ソースの変更、終了時の設定保存の失敗、日本語フォントの読み込み失敗、パニック。

### ログの出力先

- ログはログ画面（メモリ上のリングバッファ）とログファイルの両方へ出力する。
- リングバッファは最大1000件を保持する。溢れた行もログファイル側には残る。
- ログファイルの保存先はmacOSが`~/Library/Logs/VJDownloader`、Windowsが`%LOCALAPPDATA%\VJDownloader\logs`。
- ログファイルは日付ごとに`app-YYYY-MM-DD.log`として作成し、当日分へ追記する。
- ログファイルを開くたびに、先頭へ`# service.name=VJDownloader service.version=<バージョン> os.type=<darwin|windows> session.start=<RFC 3339>`
  を1行記録する。OpenTelemetryのResourceに相当し、同じ日に再起動した場合のセッション境界を示す。
- ログファイルの各行は`<RFC 3339>  <重大度>(<SeverityNumber>)  <発生源>  <本文>`形式とする。
- ログファイルへの書き込みは専用スレッドで行い、1行ごとに書き切る。クラッシュしても直前の行まで残る。
- アプリの正常終了時はキュー済みのログをすべて書き切ってからファイル出力スレッドを終了する。
- ログファイルを開けない、または書き込めない場合は、該当レコードを標準エラーへ退避する。
- 同じ行を標準出力へも複製する。リリースビルドでは受け手が無いため、ターミナルから起動した場合の確認用。
- 作成から40日より前の日付のログファイルは、起動時と日付が変わったタイミングで削除する。
  削除対象は`app-YYYY-MM-DD.log`形式のファイルのみで、それ以外のファイルには触れない。

### ログ画面

- ログ画面は独立ウィンドウ（初期サイズ760x460、最小520x320）で表示する。
- ログ一覧の各行は`<HH:mm:ss>  <重大度>  <発生源>  <本文>`形式で、等幅フォントで桁を揃えて表示する。
- 各行は重大度で色分けする。`ERROR`は赤、`WARN`は黄、`INFO`は既定色、`DEBUG`は薄いグレー。
- ログ一覧の上に絞り込み行を表示する。`最小レベル`で指定した重大度以上、`発生源`で指定した発生源のみを表示する。
  `最小レベル`の既定は`DEBUG`、`発生源`の既定は`すべて`。絞り込み条件はウィンドウを閉じても保持する。
- 絞り込みの結果が0件の場合は`条件に一致するログがありません。`を表示する。ログが1件もない場合は`ログはまだありません。`を表示する。
- ログ一覧は下方向にスクロール可能で、常に最新行が末尾に表示される。
- ログ画面には現在のログファイルのパスを`ログファイル: <パス>`として表示する。
- `表示をクリア`ボタンでログ一覧を全削除する。ログファイルは削除しない。
- `直近10分をコピー`ボタンで直近10分のログをクリップボードへコピーする。範囲の判定は実時刻で行うため、
  スリープを跨いでも画面の表示と一致する。コピー対象は絞り込み後の行に限る。
- ダウンロード成功時は`Download completed. Total time: <mm:ss or h:mm:ss>`をログ出力する。
- 入力ソース変更を監視し、日本語入力に切り替わった場合は`日本語になりました`、英字入力に切り替わった場合は
  `英字になりました`をログ出力する。それ以外の入力ソースへ切り替わった場合は`入力ソースが変更されました: <名称>`を出力する。
- 入力ソースの判定はmacOSでは入力ソースID（`ABC`を英字とみなす）、WindowsではキーボードレイアウトのLANGIDと
  IMEの変換モード（`IME_CMODE_NATIVE`の有無）で行う。

### サブウィンドウの再描画

- ストリーム再生、通信速度測定、ログはDeferred Viewportとして表示し、メインウィンドウと独立して再描画する。
- 各Deferred Viewportはウィンドウ専用の`Arc<Mutex<...>>`状態だけを共有し、`DownloaderApp`全体は共有ロックで包まない。
- ストリーム再生のMetal/Syphonオブジェクトは`Send`ではないため、Deferred ViewportのUIスレッドに固定したthread-local状態で所有する。
- ストリーム再生ウィンドウを閉じると再生処理とSyphonサーバーを停止・破棄し、再度開いた場合はSyphonを再初期化する。
- ストリームとSyphonの再描画要求は最大30fpsとし、Syphon送信バッファはウィンドウ状態内で再利用する。
- 設定と初回セットアップもDeferred Viewportとして表示し、専用の共有状態を介してメインウィンドウと独立して再描画する。
- 設定変更は自動保存せず、設定画面の保存操作をメインアプリへ要求して反映する。
- 設定画面とメインアプリの間はチャンネルで操作と結果を受け渡し、描画中の設定状態をメインアプリからロックしない。
- 設定ウィンドウの表示状態はAtomicBoolで管理し、ロック失敗を理由とする高頻度の再描画要求は行わない。

## 動画検索

設計の背景・実装ファイル・運用上の注意は`docs/search.md`にまとめる。
ここには利用者から見た振る舞いとデータ構造だけを記載する。

### 動画検索インデックス（SQLite）

- 動画検索は`~/.vjdownloader/search_index.sqlite3`のSQLiteインデックスを使用する。
- 検索対象の拡張子は`.mp4`、`.mov`、`.m4v`、`.webm`、`.mkv`とし、大文字・小文字を区別しない。
- `roots`テーブルで検索対象ルートフォルダを管理し、`files`テーブルで動画ファイル情報を管理する。
- `files`には`_path`（PK）、`root_id`、`file_name`、`file_name_norm`、`comment`、`comment_norm`、`parent_dir`、`size_bytes`、
  `modified_time`、`created_time`、`last_indexed_time`を保持する。
- `roots`には`root_id`（PK）、`root_path`、`is_enabled`、`last_scan_time`を保持する。
- `files.root_id`、`files.parent_dir`、`files.file_name_norm`、`files.modified_time`、`files.size_bytes`にインデックスを作成する。
- `comment_norm`が`NULL`の行はコメント未取得を意味し、フェーズ2の対象となる。取得済み（コメントなしを含む）の行は空文字が入る。

### 検索対象フォルダ設定

- 設定キー`search.roots`に検索対象ルートフォルダ（複数）を保存する。
- 設定画面から検索対象フォルダを追加・削除できる。
- 設定保存時に検索対象ルートをDBへ同期し、新規追加ルートはバックグラウンドでフルスキャンする。既存ルートは再走査しない。
- 起動時は検索対象ルートの同期と全ルートのフルスキャンをまとめて行い、同じルートを二重に走査しない。
- 設定画面の`全体を再インデックス`で全ルートを再スキャンできる。
- 起動時のフルスキャンはメイン画面右上、設定画面から開始したフルスキャンは設定画面の検索対象フォルダ欄に進行中表示を出す。DBへの反映完了後は同じ場所にチェックマークのアニメーションと
  `インデックスを更新しました`という完了表示を一定時間表示し、失敗時はエラー表示へ切り替える。この表示はフェーズ1（ファイル名インデックス）の完了で切り替わる。

### インデックス作成の進捗ウィンドウ

- インデックス作成中は`インデックス作成中`というミニウィンドウを独立ウィンドウとして表示する。
- フェーズ1の実行中は収集したファイル数を、フェーズ2の実行中は進捗バーと`処理済み / 総数`の件数を表示する。
- 設定画面を閉じてもミニウィンドウは残り、インデックス作成が完了すると自動的に閉じる。
- ミニウィンドウを手動で閉じた場合、そのインデックス作成の間は再表示せず、次回のインデックス作成開始時に再び表示する。
- ミニウィンドウの表示中もダウンロード・検索・変換は通常どおり操作できる。

### 検索仕様（インデックス検索）

- 検索はインデックス方式で行い、検索時にフォルダ全体のフルスキャンは行わない。
- クエリは`file_name_norm`と、動画内部のコメントを正規化した`comment_norm`に対して部分一致検索を行う。
- 空白で区切られた複数キーワードは、順序を問わずすべてが検索対象全体に含まれるAND検索として扱う。
- 非空クエリでは2段階検索を行う。
- キーワードが1つの場合、第1段階はファイル名の前方一致（`query%`）のみで検索し、完全一致を先頭に並べる。
- 第1段階が上限件数に満たない場合、第2段階でファイル名またはコメントの部分一致（`%query%`）を補完する。第1段階で一致済みのファイル名前方一致は除外し、重複を出さない。
- キーワードが複数の場合は、各キーワードを部分一致（`%term%`）でAND結合する。
- AND検索の各キーワードは、ファイル名またはコメントのどちらかに含まれていれば一致とする。
- `%`、`_`、`\`を含むクエリはLIKEエスケープしてリテラルとして扱う。
- クエリが空の場合は更新日時降順、非空の場合は名前順で返す。
- メタデータ条件として`root_id/root_path`、`parent_dir`、`modified_time`範囲、`size_bytes`範囲、`limit`、`sort`を検索APIで受け付ける。
- 1回の検索で返す件数は最大1000件とする。
- シンボリックリンク経由などOS依存のパス表現差は完全には吸収しない。

### 検索UI

- 検索結果はダウンロード一覧と同じ行UIで表示し、表示内容はファイル名のみとする。
- 検索結果行には削除ボタンを表示しない。
- 検索結果行のドラッグでmacOSネイティブのファイルドラッグを開始し、VDMXへドロップできる。
- 検索クエリが空のときは、結果リスト内に何も表示しない。
- ヒット0件時はリスト枠内に`該当するファイルはありませんでした`を表示する。
- 検索入力中の選択ハイライトは強い青色を使わず、目立たない配色にする。

### 日本語検索の扱い

- 検索用正規化はNFKC + 小文字化（英字吸収）+ カタカナのひらがな化を適用する。
- 正規化は`src/search_index/`の`normalize_for_search`で実装する。
- 同じ正規化を検索クエリ側にも適用してから照合する。
- 全角・半角カタカナはひらがなへ統一し、ひらがなの検索語で一致するようにする。
- 日本語の表記ゆれ（互換文字・結合文字）をある程度吸収するが、意味的同義語や読み仮名変換は対象外。

### 監視更新とフォールバック

- インデックス作成は2段階に分ける。フェーズ1でファイル名・サイズ・更新日時を登録し、フェーズ2で動画コメントを後から埋める。
- フェーズ1はファイルの内容を読まないため短時間で終わり、完了した時点でファイル名検索が使えるようになる。
- フェーズ2は同梱`ffprobe`で動画の`comment`および`description`タグを取得し、検索用コメントとして結合する。コメント検索は取得が済んだ動画から順に使えるようになる。
- フェーズ2はバックグラウンドで並列に実行し、ダウンロード・検索・変換と並行して動く。
- フェーズ2の途中でアプリを終了しても、次回起動時に未取得のファイルから再開する。
- ファイルサイズと更新日時が前回インデックス時から変わっていない場合、保存済みコメントを再利用して`ffprobe`の再実行を省略する。変わっている場合はフェーズ2で取得し直す。
- `ffprobe`が動作したうえで動画からコメントを読めなかった場合（壊れたファイルなど）は、そのファイルのコメントを空として扱い、警告ログを残して次のファイルへ進む。
- `ffprobe`を実行できなかった場合（未導入・起動失敗・30秒以内に無応答）は、そのファイルを取得待ちのまま残し、警告ログを残して後で取り直す。
- 取得待ちがある状態で`ffprobe`が見つからない場合、フェーズ2はコメント取得を保留して情報ログを残す。初回セットアップでの外部ツール取得が終わった時点でフェーズ2を再開する（Windowsは`ffprobe`を起動後に取得するため）。
- 監視で追加・更新された行のコメントは、その行がDBへ反映されてからフェーズ2が拾う。
- `notify`による再帰監視でルート配下の差分を取り込み、DBを更新する。
- 監視イベントはデバウンス（700ms）してまとめて処理する。
- renameは旧パス削除＋新パス追加として処理する。
- deleteはファイル削除またはディレクトリ配下削除として処理する。
- 監視エラー発生時はフォールバックとして有効ルートの再スキャンを行う。

### 並行処理とDBアクセス

- SQLite書き込みは単一ライタースレッド（キュー経由）に集約する。
- 検索は別スレッドで実行し、入力連打時は最新クエリを優先して古い要求を破棄する。
- DBはWALモードを使用し、検索と更新の並行実行時の待ちを低減する。

## プラットフォーム依存実装の分離

- Syphon出力はmacOSかつ`syphon`フィーチャー有効時のみ組み込み、Windowsでは通常のプレビュー処理を使用する。
- 変換に使うH.264エンコーダはプラットフォームで切り替える。macOSは`h264_videotoolbox`、Windowsは`libx264`を使用する。
  ダウンロード後の変換・yt-dlpの`VideoConvertor`・パイプ変換・AnimeThemes変換のすべてでこの選択結果を共有する。
- `h264_videotoolbox`が失敗したときの`libx264`での再試行はWindowsのみ行う。macOSの同梱ffmpegは外部ライブラリを
  含まないLGPLビルドで`libx264`を持たないため、再試行しても原因を隠すだけになる。
- Windowsの入力ソース判定は、前面ウィンドウが自プロセスの場合のみ行う。IMM32は他プロセスのウィンドウへ入力コンテキストを
  返さないため、他アプリが前面の間は「変化なし」として扱い、フォーカス切り替えでログを出さない。
- リリースビルドはWindowsサブシステムとしてビルドし、GUIの横にコンソールウィンドウを出さない。デバッグビルドはコンソールを残す。
- Windowsでは、アプリが起動する外部プロセス（yt-dlp / ffmpeg / ffprobe / curl / PowerShell）にコンソールウィンドウを
  割り当てない。ダウンロード・変換・検索インデックス作成・速度測定・ツール取得のいずれでも、黒いウィンドウが一瞬開くことはない。
- 実行権限ビットの検査・付与はUnix環境のみで行う。Windowsでは通常ファイルの存在を確認し、外部ツールの起動可否は既存の実行時検証で確認する。
- 対応プラットフォームはmacOSとWindowsの2つとし、それ以外のターゲットは`compile_error!`でビルドを止める。
- OS固有APIの呼び出しは`src/platform/`配下にのみ置き、それ以外のモジュール（`download/`、`stream/`、`search_index/`、`ui`など）はプラットフォーム差分を持たない。
- `src/platform/`は次の3つに分割する。
    - `common/`: 両プラットフォーム実装が共有する型のみを置く。業務ロジックは置かない。
    - `macos/`: macOS実装（AppKit / Carbon）。
    - `windows/`: Windows実装。
- 実装の切り替えは`src/platform/mod.rs`の`#[cfg_attr(target_os = ..., path = ...)]`で行い、対象外のディレクトリはコンパイル対象から除外する。
- 公開APIは`src/platform/mod.rs`でドメインごとに明示的に再エクスポートする。これにより両実装のAPI一致が強制され、片方への実装漏れはコンパイルエラーとなる。
- Windows実装のうち、フォルダ選択（`IFileOpenDialog`）とIME判定（`GetKeyboardLayout` + IMM32）は実装済みである。
- Windowsではメイン画面を右クリックすると、設定・ログ・通信速度測定・ストリーム再生・動画をMP4に変換のメニューを表示する。メニュー項目はダーク背景上で判読しやすい明るい文字色で表示する。選択すると対応するサブ画面を開き、メニューを閉じる。メニュー外クリックやEscでも閉じられる。macOSのネイティブメニューは変更しない。
- サブ画面を開く要求は、macOSではネイティブメニュー、Windowsではアプリ内UIからフラグとして渡し、UIループが回収する。この受け渡し方式は両プラットフォームで共通とする。
