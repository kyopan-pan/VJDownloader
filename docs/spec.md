...
- 取得した直リンクを同梱ffmpegの入力（複数URL時は各入力に`-reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5`と`-ss <開始位置>`）に渡す。
...
- ストリーム再生画面を閉じると再生を終了し、実行中のプロセス・プレビュー・再生位置・直リンクを初期化する。
- 再生終了時はプレビューを消去する。

### シークバー
- シークバーはウィンドウ内に現在の再生位置と総再生時間を`現在 / 総時間`形式で表示する。
- 総再生時間が取得できている再生中はシークバーをドラッグまたはクリックして任意位置へシークできる。
- シーク確定時は再生世代を更新して現行ffmpegを終了し、取得済み直リンクから指定位置で再起動する（直リンクは再解決しない）。
